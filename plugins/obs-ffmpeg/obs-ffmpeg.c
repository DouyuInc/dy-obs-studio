...
extern struct obs_source_info dyVideoSource;
extern struct obs_source_info dyNetVideoSource;
...
static bool nvcuda_in_symlink() {
	// 注册表路径HKEY_LOCAL_MACHINE\SYSTEM\ControlSet001\Services\nvlddmkm
	bool isSymlnk = false;
	HKEY keyResult = NULL;
	DWORD dwLen = 0;
	TCHAR *pPath = NULL;
	...
	do 
	{
		...
		if (ERROR_SUCCESS != RegGetValueW(keyResult, 0, L"ImagePath", 2, 0, 0, &dwLen)) {
			break;
		}

		pPath = (TCHAR*)malloc(dwLen + sizeof(TCHAR));
		...
		pRealPath = (TCHAR*)malloc((dwLenWin + dwLen - 10) * sizeof(TCHAR));
		...
		if (dwAttr != FILE_INVALID_FILE_ID) {
			isSymlnk = dwAttr & FILE_ATTRIBUTE_REPARSE_POINT;
			break;
		}
...
{
	if (nvcuda_in_symlink()) {
		blog(LOG_ERROR, "[ERROR] nvenc not support, because nvcuda path is symlink!!!");
		return false;
	}
...
{
    obs_register_source(&dyVideoSource);
    obs_register_source(&dyNetVideoSource);
