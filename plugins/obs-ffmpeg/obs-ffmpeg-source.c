...
/* TODO Chensi 确认这部分修改是否有必要，[直播伴侣海外版][fix]修复obs视频源多线程bug，现象：添加网络源不正常播放，但是点设置后正常播放
    struct ffmpeg_source *s = data;
    //if (s->destroy_media) {
    //    if (s->media_valid) {
...
static void ffmpeg_source_update(void *data, obs_data_t *settings)
{
	...
	tmp.width = 1920;
	tmp.height = 1080;
// 	tmp.format = VIDEO_FORMAT_I420;
	obs_source_output_video(s->source, &tmp);
}
