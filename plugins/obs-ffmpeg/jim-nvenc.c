...
const int g_defaultGopIntervalInSec = 3;
			 ...
			 bool psycho_aq)
{
	...
	uint32_t gop_size =
		(keyint_sec) ? keyint_sec * voi->fps_num / voi->fps_den : g_defaultGopIntervalInSec * voi->fps_num / voi->fps_den;
...
fail:
	info("jimnvenc not supported, falling back to ffmpeg");
	nvenc_destroy(enc);
	return NULL;
}
