...
#ifdef _WIN32
#define FFMPEG_MUX "DYffmpeg-mux.exe"
#else
#define FFMPEG_MUX "DYffmpeg-mux"
#endif
...
// TODO Chensi 如果下面的一段代码不需要了，这里的include代码也不需要
#include <windows.h>
...
static bool ffmpeg_mux_start(void *data)
{
    ...
    /* TODO Chensi 核实一下现在MD编译这段内容是否能正常编译通过
    // _wfopen有bug，在MT编译时，文件句柄在判定读写状态时会异常，替换成fopen，但是fopen只能支持英文文件名，所以还是换成Windows原生吧，蛋疼
    wchar_t *wpath = NULL;
    HANDLE hFile = NULL;
    if (path) {
        os_utf8_to_wcs_ptr(path, 0, &wpath);
        hFile = CreateFileW(wpath, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
        bfree(wpath);
    }
