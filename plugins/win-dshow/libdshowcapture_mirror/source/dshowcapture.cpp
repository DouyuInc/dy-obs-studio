...
bool EnumVideoDevicesReal(std::vector<VideoDevice> &devices)
{
	devices.clear();
...
std::vector<VideoDevice> sVideoDevice;

bool Device::EnumVideoDevices(std::vector<VideoDevice> &devices)
{
	bool ret = false;
	if (sVideoDevice.empty()) {
		ret = EnumVideoDevicesReal(sVideoDevice);
	}
	if (sVideoDevice.empty()) {
		return false;
	}
	devices.clear();
	devices = sVideoDevice;
	return true;
}
...
bool EnumAudioDevicesReal(vector<AudioDevice> &devices)
{
	devices.clear();
...
vector<AudioDevice> sAudioDevice;

bool Device::EnumAudioDevices(vector<AudioDevice> &devices)
{
	if (sAudioDevice.empty()) {
		EnumAudioDevicesReal(sAudioDevice);
	}
	if (sAudioDevice.empty()) {
		return false;
	}
	devices.clear();
	devices = sAudioDevice;
	return true;
}
