...
//	dstr_free(&encoder_name);
    ...
    obs_data_t* settings = obs_output_get_settings(context);
    obs_data_t* dyMetaDatas = obs_data_get_obj(settings, "DYMetaData");
    if (dyMetaDatas)
    {
        obs_data_item_t *item = obs_data_first(dyMetaDatas);
