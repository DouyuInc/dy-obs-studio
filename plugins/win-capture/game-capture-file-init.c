...
#define get_programdata_path(path, subpath)                        \
	do {                                                       \
		SHGetFolderPathW(NULL, CSIDL_LOCAL_APPDATA, NULL, \
...
char *get_hook_path(bool b64)
{
	...
	get_programdata_path(path, L"DYToolEx\\\Data\\plugins\\DYWinCapture\\");
...
static bool update_hook_file(bool b64)
{
	...
	get_programdata_path(temp, L"DYToolEx\\Data\\plugins\\DYWinCapture\\");
	...
	if (!file_exists(dst) /*|| !file_exists(dst_json)*/) {
		//bool ret= CreateDirectoryW(temp, NULL);
		char tempShort[256];
		sprintf(tempShort, "%ws", temp);
		bool result = MakeSureDirectoryPathExists((PCSTR) (tempShort));
		if (!result)
		{
			return false;
		}
