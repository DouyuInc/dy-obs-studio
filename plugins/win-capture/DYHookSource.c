...
#define SETTING_COMPATIBILITY    "share_mem"
...
#define SETTING_ANTI_CHEAT_HOOK  "anti_cheat"
...
#define TEXT_GAME_CAPTURE        obs_module_text("DYHookSource")
...
static void game_capture_update(void *data, obs_data_t *settings)
{
    ...
    const char *titleName = obs_data_get_string(settings, "title_name");
    const char *className = obs_data_get_string(settings, "class_name");
    const char *processName = obs_data_get_string(settings, "process_name");
    gc->config.cursor = obs_data_get_bool(settings, SETTING_CURSOR);
    if (s_cmp(titleName, gc->title.array)==0 &&
        s_cmp(className,gc->class.array) == 0
        )
    {
        return;
    }

    int len = strlen(titleName) + strlen(className) + strlen(processName) + 3;
    char *window = malloc(len);
    memset(window, '\0', sizeof(window));
    strcat(window, titleName);
    strcat(window, ":");
    strcat(window, className);
    strcat(window, ":");
    strcat(window, processName);
...
static void try_hook(struct game_capture *gc)
{
    ...
    if (gc->next_window) {
        ...
        obs_data_set_int(gc->settings, c_keyHWnd, (int)gc->next_window);
    }
    else {
        gc->active = false;
    }
...
static void checkTexture(void *data)
{
	struct game_capture *gc = data;
	if (gc->hasSuccessed)
	{
	...
	if (isTextureValid(gc->texture, !gc->hasSuccessed))
	{
		gc->hasSuccessed = true;
	}
}
...
static void game_capture_defaults(obs_data_t *settings)
{
    ...
    obs_data_set_default_string(settings, "title_name", "");
    obs_data_set_default_string(settings, "class_name", "");
    obs_data_set_default_string(settings, "game_name", "");
    obs_data_set_default_string(settings, "process_name", "");
    ...
    obs_data_set_default_bool(settings, SETTING_CURSOR, false);
...
static bool check_window_property_setting(obs_properties_t *ppts, obs_property_t *p,
...
{
	struct game_capture *gc = data;
	return gc->hasSuccessed;
}

struct obs_source_info dyHookSourceInfo = {
    .id = "DY_Hook",
