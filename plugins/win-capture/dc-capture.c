...
static void draw_texture_subregion(struct dc_capture *capture, gs_effect_t *effect, uint32_t ox, uint32_t oy, uint32_t cx, uint32_t cy)
{
    gs_texture_t *texture = capture->texture;
    gs_technique_t *tech = gs_effect_get_technique(effect, "Draw");
    gs_eparam_t *image = gs_effect_get_param_by_name(effect, "image");
    ...
    for (size_t i = 0; i < passes; i++)
    {
        ...
        {
            if (capture->compatibility)
                gs_draw_sprite_subregion(texture, GS_FLIP_V, ox, oy, cx, cy);
            else
...
void dc_capture_render_subregion(struct dc_capture *capture, gs_effect_t *effect,uint32_t ox,uint32_t oy, uint32_t cx, uint32_t cy)
{
    if (capture->valid && capture->texture_written)
        draw_texture_subregion(capture, effect, ox, oy, cx, cy);
}