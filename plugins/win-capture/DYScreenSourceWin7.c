	...
	obs_data_t *settings)
{
	...
	monitor.desired_id = (int)obs_data_get_int(settings, c_keyIndex);
	...
	obs_data_t *settings)
{
	...
	blog(LOG_INFO, "source settings monitor=%d", monitor);
    ...
    bool compatibility = obs_data_get_bool(settings, "compatibility");
    if (capture->compatibility != compatibility)
    {
        initNeeded = true;
...
{
	obs_data_set_default_int(settings, c_keyIndex, 0);
	obs_data_set_default_bool(settings, c_keyCaptureCursor, true);
...
static void monitor_capture_tick(void *data, float seconds)
{
	...
	if (capture->cx <= 0) {
		capture->cx = capture->data.width;
	}
	if (capture->cy <= 0) {
		capture->cy = capture->data.height;
	}
...
{
	struct monitor_capture *capture = data;
	dc_capture_render_subregion(&capture->data,
		obs_get_base_effect(OBS_EFFECT_OPAQUE), capture->ox, capture->oy, capture->cx, capture->cy);
...
struct obs_source_info dyScreenSourceWin7Info = {
	.id = "DY_FullScreen",
...
static const char *screenShotWin7Getname(void *unused)
{
	UNUSED_PARAMETER(unused);
	return "截屏";
}

struct obs_source_info dyScreenShotSourceWin7Info = {
	.id = "DY_ScreenShot",
	.type = OBS_SOURCE_TYPE_INPUT,
	.output_flags = OBS_SOURCE_VIDEO | OBS_SOURCE_CUSTOM_DRAW |
			OBS_SOURCE_DO_NOT_DUPLICATE,
	.get_name = screenShotWin7Getname,
	.create = monitor_capture_create,
	.destroy = monitor_capture_destroy,
	.video_render = monitor_capture_render,
	.video_tick = monitor_capture_tick,
