...
class WASAPISource {
	...
	//vst demo
	obs_source_info vstInfo = { 0 };
	void* vstPluginData = nullptr;
	obs_source_t* vstFilter = nullptr;
	const char* vstFilterName="";
	const char* vstFilterID="";


	obs_source_info getVSTInfo();
...
void WASAPISource::InitClient()
{
	...
	if (FAILED(res)) {
		...
		if (wfex->nChannels > 2){
			...
			InitFormat(wfex);
			res = client->Initialize(AUDCLNT_SHAREMODE_SHARED, flags, BUFFER_TIME_100NS, 0, wfex, NULL);
			if (FAILED(res)){
				if (AUDCLNT_E_ALREADY_INITIALIZED == res){
					client->Reset();
					res = client->Initialize(AUDCLNT_SHAREMODE_SHARED, flags, BUFFER_TIME_100NS, 0, wfex, NULL);
					if (FAILED(res)){
