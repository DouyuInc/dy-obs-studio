...
//void SendTestData(struct obs_output *output, uint8_t* data, size_t len) {
//	struct encoder_packet pkt = { 0 };
//	pkt.timebase_num = output->audio_encoders[0]->timebase_num;
//	pkt.timebase_den = output->audio_encoders[0]->timebase_den;
//	pkt.encoder = output->audio_encoders[0];
//	pkt.data = data;
//	pkt.size = len;
//	pkt.keyframe = true;
...
//	pkt.track_idx = 0;
//	pkt.pts = _time64(0);
//	//send_off_encoder_packet(encoder, true, true, &pkt);
...
static inline void send_interleaved(struct obs_output *output)
{
	...
	// 为了兼容斗鱼的语音直播而做的更改
	obs_encoder_t* encoder = output->video_encoder;
	bool disableVideo = false;
	if (encoder)
	{
		obs_data_t *settings = obs_encoder_get_settings(encoder);
		obs_data_release(settings);
		obs_data_set_default_bool(settings, "disable_video_encode", false);
		disableVideo = obs_data_get_bool(settings, "disable_video_encode");
	}
...
static inline void signal_reconnect(struct obs_output *output)
{
	...
	blog(LOG_INFO, "reconnect failed: last_error=%s", output->last_error_message ? output->last_error_message : "");
