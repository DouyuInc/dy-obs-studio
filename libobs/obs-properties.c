...
struct douyu_data {
    DARRAY(struct douyu_item) items;
    enum obs_douyu_type type;
    enum obs_douyu_format format;
};
...
//////////////////////////////////////////////////////////////////////////
// Property Douyu [1/19/2021 chensi2]
static inline void douyu_item_free(struct douyu_data *data,
    struct douyu_item *item)
{
    bfree(item->name);
    bfree(item->ext);
    if (data->format == OBS_DOUYU_FORMAT_STRING)
        bfree(item->str);
}

static inline void douyu_data_free(struct douyu_data *data)
{
    for (size_t i = 0; i < data->items.num; i++)
        douyu_item_free(data, data->items.array + i);
...
static inline size_t get_property_size(enum obs_property_type type)
{
	...
	case OBS_PROPERTY_COLOR_ALPHA:
		return 0;
    case OBS_PROPERTY_DOUYU:
        return sizeof(struct douyu_data);
	}
...
obs_property_t *obs_properties_add_douyu(obs_properties_t *props,
    const char *name, const char *desc, enum obs_douyu_type type, enum obs_douyu_format format)
{
    if (!props || has_prop(props, name))
        return NULL;

    struct obs_property *p = new_prop(props, name, desc, OBS_PROPERTY_DOUYU);
    struct douyu_data *data = get_property_data(p);
    data->format = format;
    data->type = type;
...
//////////////////////////////////////////////////////////////////////////
// Property Douyu [1/19/2021 chensi2]
static inline bool is_douyu(struct obs_property *p)
{
    return p->type == OBS_PROPERTY_DOUYU;
}

static inline struct douyu_data *get_douyu_data(struct obs_property *p)
{
    if (!p || !is_douyu(p))
        return NULL;
...
static inline struct douyu_data *get_douyu_fmt_data(struct obs_property *p,
    enum obs_douyu_format format)
{
    struct douyu_data *data = get_douyu_data(p);
    return (data && data->format == format) ? data : NULL;
}

enum obs_douyu_type obs_property_douyu_type(obs_property_t *p)
{
    struct douyu_data *data = get_douyu_data(p);
    return data ? data->type : OBS_DOUYU_TYPE_INVALID;
}

enum obs_douyu_format obs_property_douyu_format(obs_property_t *p)
{
    struct douyu_data *data = get_douyu_data(p);
    return data ? data->format : OBS_DOUYU_TYPE_INVALID;
}


void obs_property_douyu_clear(obs_property_t *p)
{
    struct douyu_data *data = get_douyu_data(p);
    if (data)
        douyu_data_free(data);
}

static size_t add_douyu_item(struct douyu_data *data, const char *name, const char* ext,
    const void *val)
{
    struct douyu_item item = { NULL };
    item.name = bstrdup(name);
    item.ext = bstrdup(ext);

    if (data->format == OBS_DOUYU_FORMAT_INT)
        item.ll = *(const long long *)val;
    else if (data->format == OBS_DOUYU_FORMAT_FLOAT)
        item.d = *(const double *)val;
    else
        item.str = bstrdup(val);
...
static void insert_douyu_item(struct douyu_data *data, size_t idx, const char *name, const char* ext,
    const void *val)
{
    struct douyu_item item = { NULL };
    item.name = bstrdup(name);
...
size_t obs_property_douyu_add_string(obs_property_t *p, const char *name, const char* ext,
    const char *val)
{
    struct douyu_data *data = get_douyu_data(p);
    if (data && data->format == OBS_DOUYU_FORMAT_STRING)
        return add_douyu_item(data, name, ext, val);
    return 0;
}

size_t obs_property_douyu_add_int(obs_property_t *p, const char *name, const char* ext,
    long long val)
{
    struct douyu_data *data = get_douyu_data(p);
    if (data && data->format == OBS_DOUYU_FORMAT_INT)
        return add_douyu_item(data, name, ext, &val);
    return 0;
}

size_t obs_property_douyu_add_float(obs_property_t *p, const char *name, const char* ext,
    double val)
{
    struct douyu_data *data = get_douyu_data(p);
    if (data && data->format == OBS_DOUYU_FORMAT_FLOAT)
        return add_douyu_item(data, name, ext, &val);
    return 0;
}

void obs_property_douyu_insert_string(obs_property_t *p, size_t idx,
    const char *name, const char* ext, const char *val)
{
    struct douyu_data *data = get_douyu_data(p);
    if (data && data->format == OBS_DOUYU_FORMAT_STRING)
        insert_douyu_item(data, idx, name, ext, val);
}

void obs_property_douyu_insert_int(obs_property_t *p, size_t idx,
    const char *name, const char* ext, long long val)
{
    struct douyu_data *data = get_douyu_data(p);
    if (data && data->format == OBS_DOUYU_FORMAT_INT)
        insert_douyu_item(data, idx, name, ext, &val);
...
void obs_property_douyu_insert_float(obs_property_t *p, size_t idx,
    const char *name, const char* ext, double val)
{
    struct douyu_data *data = get_douyu_data(p);
    if (data && data->format == OBS_DOUYU_FORMAT_FLOAT)
        insert_douyu_item(data, idx, name, ext, &val);
...
void obs_property_douyu_item_disable(obs_property_t *p, size_t idx,
    bool disabled)
{
    struct douyu_data *data = get_douyu_data(p);
    if (!data || idx >= data->items.num)
...
bool obs_property_douyu_item_disabled(obs_property_t *p, size_t idx)
{
    struct douyu_data *data = get_douyu_data(p);
    return (data && idx < data->items.num) ? data->items.array[idx].disabled
        : false;
...
void obs_property_douyu_item_remove(obs_property_t *p, size_t idx)
{
    struct douyu_data *data = get_douyu_data(p);
    if (data && idx < data->items.num) {
        douyu_item_free(data, data->items.array + idx);
        da_erase(data->items, idx);
    }
}

size_t obs_property_douyu_item_count(obs_property_t *p)
{
    struct douyu_data *data = get_douyu_data(p);
    return data ? data->items.num : 0;
}

const char *obs_property_douyu_item_name(obs_property_t *p, size_t idx)
{
    struct douyu_data *data = get_douyu_data(p);
    return (data && idx < data->items.num) ? data->items.array[idx].name
        : NULL;
...
const char *obs_property_douyu_item_ext(obs_property_t *p, size_t idx)
{
    struct douyu_data *data = get_douyu_data(p);
    return (data && idx < data->items.num) ? data->items.array[idx].ext
        : NULL;
}

const char *obs_property_douyu_item_string(obs_property_t *p, size_t idx)
{
    struct douyu_data *data = get_douyu_fmt_data(p, OBS_COMBO_FORMAT_STRING);
    return (data && idx < data->items.num) ? data->items.array[idx].str
        : NULL;
}

long long obs_property_douyu_item_int(obs_property_t *p, size_t idx)
{
    struct douyu_data *data = get_douyu_fmt_data(p, OBS_COMBO_FORMAT_INT);
    return (data && idx < data->items.num) ? data->items.array[idx].ll : 0;
}

double obs_property_douyu_item_float(obs_property_t *p, size_t idx)
{
    struct douyu_data *data = get_douyu_fmt_data(p, OBS_COMBO_FORMAT_FLOAT);
    return (data && idx < data->items.num) ? data->items.array[idx].d : 0.0;
}
