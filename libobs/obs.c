...
static bool enum_source(void *unused, obs_source_t *source)
{
	if (source && source->monitor)
	{
		blog(LOG_INFO, "shutdown, but source(%s, %s) monitor exist.",
			obs_source_get_name(source) ? obs_source_get_name(source) : "",
			obs_source_get_unversioned_id(source) ? obs_source_get_unversioned_id(source) : "");
	}
...
static void obs_free_data(void)
{
	...
	// 遍历一下所有的source
	obs_enum_sources(enum_source, 0);
...
static obs_source_t *obs_load_source_type(obs_data_t *source_data)
{
	...
	// 在load过程中，添加一个字段，表示loading过程，很多素材需要区分load和非load，干不同的活
	obs_data_set_bool(settings, "dy_loading", true);
