...
static inline void obs_set_buffer(obs_data_t *data, obs_data_item_t **item,
	const char *name, uint8_t *val, size_t len,
	set_item_t set_item_)
{
	if (!val || !len)
		val = 0;
	set_item_(data, item, name, val, len, OBS_DATA_BUFFER);
}
...
void obs_data_set_buffer(obs_data_t *data, const char *name,
	uint8_t *val, size_t len)
{
	obs_set_buffer(data, NULL, name, val, len, set_item);
}
...
void obs_data_get_buffer(obs_data_t *data, const char *name,
	uint8_t **buf, size_t* len) {
	obs_data_item_get_buffer(get_item(data, name), buf, len);
}
...
static inline void data_item_get_buffer(obs_data_item_t *item,
	get_data_t get_data, uint8_t** buf, size_t* len) {
	if (item_valid(item, OBS_DATA_BUFFER) && get_data(item)) {
		*buf = get_data(item);
		*len = item->data_size;
	}
}
...
void obs_data_item_get_buffer(obs_data_item_t *item, uint8_t** buf, size_t* len) {
	data_item_get_buffer(item, get_item_data, buf, len);
}
