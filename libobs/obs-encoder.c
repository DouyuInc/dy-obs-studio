...
// 			if (gap > 100) {
// 				gap = 0;
// 				uint8_t testBuf[12] = { 0xd4, 0x00, 0x02, 0x46, 0x8a, 0xcf, 0x12, 0xc8, 0xde, 0xea, 0xf2, 0x25 };
// 				SendTestData(encoder, testBuf, sizeof(testBuf));
// 			}
		}
...
bool do_encode(struct obs_encoder *encoder, struct encoder_frame *frame)
{
	...
	pkt.encoder = encoder;
	pkt.sys_time_ms = frame->timestamp_ms;
...
static void receive_video(void *param, struct video_data *frame)
{
	...
	enc_frame.timestamp_ms = frame->timestamp / 1000000;

	// 为了兼容斗鱼的语音直播，这里需要把video encode给禁用掉
	obs_data_t *settings = obs_encoder_get_settings(encoder);
	obs_data_release(settings);
	obs_data_set_default_bool(settings, "disable_video_encode", false);
	if (!obs_data_get_bool(settings, "disable_video_encode"))
	{
