...
void *obs_source_get_context_data(obs_source_t *source)
{
    return obs_source_valid(source, "obs_source_get_context_data")
        ? source->context.data
        : NULL;
}
...
void obs_source_destroy(struct obs_source *source)
{
	...
	// 退出的时候，找一下到底谁还没有释放这里的monitor
	if (!(obs->data.valid) && source->monitor)
	{
		blog(LOG_INFO, "shutdown, but source(%s, %s) monitor not null.", obs_source_get_name(source), obs_source_get_unversioned_id(source));
	}
...
long obs_source_get_weak_ref(obs_source_t *source)
{
	if (!source)
		return 0;

	return source->control->ref.refs;
}
...
void obs_source_release_audio(obs_source_t *source)
{
	if (!source)
		return;
...
/* called only in render thread */
void obs_source_update_directly(obs_source_t *source, obs_data_t *settings)
{
    if (!obs_source_valid(source, "obs_source_update"))
        return;
...
void obs_source_video_tick(obs_source_t *source, float seconds)
{
    ...
    // 首先tick插件，然后再获取异步数据，提高实时性 [7/30/2020 shijie]
...
void obs_source_render_async_video(obs_source_t *source)
{
...
size_t obs_source_get_async_cache_size(obs_source_t *source)
{
	return obs_source_valid(source, "obs_source_get_async_cache_size")
		? source->async_cache.num
		: 0;
}

size_t obs_source_get_audio_cache_size(obs_source_t *source)
{
	return obs_source_valid(source, "obs_source_get_audio_cache_size")
		? source->audio_storage_size
		: 0;
}
