...
struct winrt_capture {
			      ...
			      winrt::Windows::Foundation::IInspectable const &)
	{
        ...
        if (!IsWindow(window) || IsIconic(window))	// TODO Chensi，OBS代码没有这段if，核实一下是否需要
        {
            obs_leave_graphics();
            return;
        }
...
{
	blog(LOG_ERROR, "winrt_capture_free %d", capture->window);
    if (capture) {
...
winrt_get_texture(const struct winrt_capture *capture)
{
